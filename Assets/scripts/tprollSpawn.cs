...
public class tprollSpawn : MonoBehaviour {
	...
	void Update () {
        if (rb.velocity.x < 1800)
        {
         //   rb.AddForce(-transform.up * 2);
       //     rb.AddForce(transform.up * 777);
           rb.AddRelativeForce(Vector3.right * -(1800));
        }
    }
    ...
    //copied and cleaned up from enemyBoxGas.cs
